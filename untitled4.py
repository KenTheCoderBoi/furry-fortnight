...
def getBatterylife():
    ...
    if battery.secsleft==BatteryTime.POWER_TIME_UNLIMITED:
        battery_life['text']="The battery is plugged,unplug it"
    if battery.secsleft==BatteryTime.POWER_TIME_UNKNOWN:
        battery_life['text']="run program again"
    else:
        battery_life['text']="Battery life : "+battery_left
        root.after(100, getBatterylife)
        root.after(100, print(str(hours)+" hours :"+str(minutes)+" minutes :"+str(seconds)+" seconds"))
getBatterylife()
root.mainloop()
